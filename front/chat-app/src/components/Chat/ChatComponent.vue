<template>
    <div :class="['main', { 'main-expanded': !isSidebarVisible }]">
        ...
        <form class="form" @submit.prevent="sendMessage">
          ...
          <button type="submit" class="send" :disabled="!current_chat_id" @click="$emit('send-message',message_input)">Отправить</button>
        </form>
      </div>
</template>
  ...
  export default{
    ...
    methods:{
      get_user_name(user_id){
        ...
        for (let index = 0; index < this.current_chat_users.length; index++) {
          if (this.current_chat_users[index].id == user_id){
            return this.current_chat_users[index].name;
          }
        }
        return "null";
      }
